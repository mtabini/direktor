...
Task.prototype.clone = function cloneTask(options) {
    var opts = extend(options, this.connectionOptions);
...
Task.prototype._perform = function _perform(callback) {
    ...
    async.eachSeries(
        [ this.before, this.commands, this.after ].filter(function(item) { return !!item; }),
    ...
    async.series(
        ...
        function(err) {
            if (err) {
                err.task = _this;
                if (_this.error) {
                    _this._performCommandOrSequence.call(_this, _this.error, function() {
                        reportBack(err);
                    });
                } else {
                    reportBack(err);
